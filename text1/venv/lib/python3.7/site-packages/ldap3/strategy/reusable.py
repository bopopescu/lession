...
class ReusableStrategy(BaseStrategy):
    ...
    class PooledConnectionThread(Thread):
        ...
        def run(self):
            ...
            while not terminate:
                try:
                    counter, message_type, request, controls = pool.request_queue.get(block=True, timeout=self.main_connection.strategy.pool.keepalive)
    ...
    class PooledConnectionWorker(object):
        ...
        def __init__(self, connection, request_queue):
            ...
            self.task_counter = 0
            self.new_connection()
            self.thread = ReusableStrategy.PooledConnectionThread(self, self.main_connection)
        ...
        def new_connection(self):
            ...
            if self.main_connection.auto_bind and self.main_connection.auto_bind not in [AUTO_BIND_NONE, AUTO_BIND_DEFAULT]:
                if log_enabled(BASIC):
    ...
    def validate_bind(self, controls):
        ...
        if (self.connection.user != self.pool.main_connection.user or
                self.connection.password != self.pool.main_connection.password or
                self.connection.authentication != self.pool.main_connection.authentication or
                self.connection.sasl_mechanism != self.pool.main_connection.sasl_mechanism or
                self.connection.sasl_credentials != self.pool.main_connection.sasl_credentials):
            self.pool.main_connection.user = self.connection.user
            self.pool.main_connection.password = self.connection.password
            self.pool.main_connection.authentication = self.connection.authentication
            self.pool.main_connection.sasl_mechanism = self.connection.sasl_mechanism
            self.pool.main_connection.sasl_credentials = self.connection.sasl_credentials
